...
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.serverVersion = function () {
    ...
    return {serverVersionResult: {'string': retVal}}
};
 ...
 */
webService.QBWebConnectorSvc.QBWebConnectorSvcSoap.sendRequestXML = async function(args, callback) {
    var requestXML = '';
    var totalRequests = requestQueue.length;
    var ticket = args ? args.ticket : '';

    if (counter < totalRequests) {
        requestXML = await qbXMLHandler.buildXML(ticket, requestQueue[counter]);
        counter += 1;
    } else {
        requestXML = '';
        counter = 0;
